 ...
 */
class TypeTranslateFilter implements Filter
{
    function __construct(private $class_namespace = null)
    {
    }
    /**
     * @param Builder $query
    ...
    public function __invoke(Builder $query, $value, string $property)
    {
        if (is_array($value)) {
            ...
            if (array_key_exists('base', $first)) {
                if ($first['base'] == 'and') {
                    $query->whereTranslation($property, $first['value'], $this->class_namespace);
                } else {
                    $query->orWhereTranslation($property, $first['value'], $this->class_namespace);
                }
            } else {
                $query->whereTranslation($property, $first['value'], $this->class_namespace);
            }
            ...
            foreach ($value as $key => $data) {
                $query->orWhereTranslation($property, $data['value'], $this->class_namespace);
            }
        }
    }
}
